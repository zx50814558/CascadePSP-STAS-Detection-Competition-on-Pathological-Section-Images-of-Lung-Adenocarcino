...
if para['id'].lower() != 'null':
    long_id = '%s_%s' % (para['id'],datetime.datetime.now().strftime('%Y-%m-%d_%H_%M_%S'))
else:
    long_id = None
...
# Construct model
model = PSPNet(sizes=(1, 2, 3, 6), psp_size=2048, deep_features_size=1024, backend='resnet50')  # resnet50 PS_Transformer
...
# big_dir = os.path.join('data', 'BIG')
# relabeled_pascal_dir = os.path.join('data', 'BIG')


train_dir_dataset = OnlineTransformDataset(train_dir, method=1, perturb=True)
...
# big_dataset = OnlineTransformDataset(big_dir, method=1, perturb=True)
# relabeled_pascal_dataset = OnlineTransformDataset(relabeled_pascal_dir, method=1, perturb=True)


print('train_dir dataset size: ', len(train_dir_dataset))
...
# print('relabeled_pascal dataset size: ', len(relabeled_pascal_dataset))


train_dataset = ConcatDataset([train_dir_dataset])

# train_dataset = ConcatDataset([fss_dataset, duts_tr_dataset, duts_te_dataset, ecssd_dataset, msra_dataset, big_dataset, relabeled_pascal_dataset, train_dir_dataset])
