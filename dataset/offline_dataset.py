...
class OfflineDataset(Dataset):
    def __init__(self, root, in_memory=False, need_name=False, resize=False, do_crop=False):
        ...
        if self.resize:
            self.resize_bi = lambda x: x.resize((224, 224), Image.NEAREST)  # Image.BILINEAR  BICUBIC
            self.resize_nr = lambda x: x.resize((224, 224), Image.NEAREST)  # Image.NEAREST
        else:
    ...
    def __getitem__(self, idx):
        if self.in_memory:
            ...
            im, seg = self.load_tuple(self.im_list[idx])
        ...
        if self.need_name:
            ...
            return im, seg, os.path.basename(self.im_list[idx][:-7])
        else:
            # return im, seg, gt
            return im, seg
