...
def process_high_res_im(model, im, seg, para, name=None, aggre_device='cpu:0'):
    ...
    for new_size in [max(h, w)]:
        ...
        r_pred_224 = (F.interpolate(pred_224, size=(h, w), mode='area')>0.5).float()*2-1  # mode='bilinear' , align_corners=False
        r_pred_56 = F.interpolate(pred_56, size=(h, w), mode='area')*2-1  # mode='bilinear' , align_corners=False
    ...
    _, _, h, w = seg.shape
    images = {}
    images['pred_224'] = F.interpolate(pred_224, size=(h, w), mode='area')  #  mode='bilinear' , align_corners=False
